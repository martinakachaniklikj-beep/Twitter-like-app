...
export function ChatInput({ onSend, conversationId }: ChatInputProps) {
  ...
  const send = useCallback(() => {
    ...
    if (trimmed.length > MAX_MESSAGE_TEXT_LENGTH) {
      console.warn(`Message is too long. Maximum length is ${MAX_MESSAGE_TEXT_LENGTH} characters.`);
      return;
    }
  ...
  const handleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (tooLargeCount > 0) {
      console.warn(
        `Skipped ${tooLargeCount} file(s) larger than ${(
  ...
  return (
    <ChatInputRoot style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <Popover open={kittyOpen} onOpenChange={setKittyOpen}>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="icon" aria-label="Kitty Bot helpers">
            <Cat size={18} />
          </Button>
        </PopoverTrigger>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            ...
            >
              {isTranscribing
                ...
                : 'Dictate with voice (speech → text)'}
            </KittyPopoverButton>
          </div>
        </PopoverContent>
      </Popover>
            ...
            {state.files.map((file: File, index: number) => {
              ...
              return (
                <FilePreviewItem key={key}>
                  ...
                  {!isImage && !isVideo && !isAudio && <FilePlaceholder>FILE</FilePlaceholder>}
                  <FileInfo>
